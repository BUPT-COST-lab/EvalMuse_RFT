...
def process_sample(sample, i):
    ...
    try:
        ...
        print(f"Saved results for sample {i+2854} to {output_file}")
        
    except Exception as e:
        print(f"Error: {e}")

# 使用ThreadPoolExecutor并发处理样本
with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
    futures = [executor.submit(process_sample, sample, i) for i, sample in enumerate(samples)]
